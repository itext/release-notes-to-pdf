...
{
    public class UniqueHandlerInstancePdfADocument : PdfADocument {
        private readonly ICollection<IEventHandler> documentHandlers =
            (ICollection<IEventHandler>)new LinkedHashSet<IEventHandler>();
        public UniqueHandlerInstancePdfADocument(PdfWriter writer, PdfAConformance aConformance, PdfOutputIntent outputIntent) : base(writer, aConformance, outputIntent)
        {
        }
